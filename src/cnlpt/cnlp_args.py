...
class CnlpTrainingArguments(TrainingArguments):
    ...
    model_selection_score: str = field(
        default=None, metadata={"help": "Score to use in evaluation. Should be one of acc, f1, acc_and_f1, recall, or precision."}
    )
    model_selection_label: Union[int, str, List[int], List[str]] = field(
        default=None, metadata={"help": "Class whose score should be used in evalutation. Should be an integer if scores are indexed, or a string if they are labeled by name."}
    )

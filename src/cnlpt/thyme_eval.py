...
def main(args):
    if len(args) < 3:
        sys.stderr.write(
            "Required arguments: <input directory> <rest host> <output directory> [entity xml dir]*\n"
        )
        sys.exit(-1)
    ...
    if len(args) > 3:
        ...
        for xml_ind, xml_dir in enumerate(args[3:]):
            ...
            for sub_dir, text_name, xml_names in anafora.walk(xml_dir):
                ...
                anafora_input_data = AnaforaData.from_file(
                    join(xml_dir, sub_dir, xml_name)
                )
            ...
            for sentence_ind, sentence in enumerate(sentences):
                ...
                if tb_tokenize:
                    ...
                    if ('"' in sent_txt) or ("''" in sent_txt):
                        ...
                        matched = [
                            m.group() for m in re.finditer(r"``|'{2}|\"", sent_txt)
                        ]
        ...
        for sent_ind, sentence in enumerate(sentences):
            ...
            if not given_entities:
                for event in sent_events:
                    ...
                    dtr = event["dtr"]
                    event_start_offset = (
                        token_spans[begin_token_ind][0] + sentence.begin
                    )
                ...
                for timex in sent_timexes:
                    ...
                    if (
                        meta_rev_loc >= 0
                        and timex_start_offset > meta_rev_loc
                        and timex_end_offset < meta_rev_end
                    ):
                        timex_ids.append(-1)

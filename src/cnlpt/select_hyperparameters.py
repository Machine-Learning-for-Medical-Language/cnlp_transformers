...
def get_job_from_hyperparameters(hyperparameters, indices):
    ...
    if len(indices) == 1:
        ...
        raise ValueError(f'Expected {len(paired_lists)} indices but received {len(indices)}')


@click.command()
@click.argument('hyperparameters', type=click.Path(path_type=Path, exists=True, dir_okay=False))
@click.argument('indices', type=int, nargs=-1, required=True)
@click.option('--default-args',

...
def preprocess_raw_data(
    columns: dict[str, Union[list[str], list[int], list[float]]],
    ...
    if not inference_only:
        ...
        labels = [
            _preprocess_raw_labels(raw=columns[task.name], task=task) for task in tasks
        ]
...
def _build_labels_for_task(
    ...
    if task.type == TAGGING:
        return get_tagging_labels(task, tokenized_input, labels)
    elif task.type == RELATIONS:
        return get_relex_labels(task, tokenized_input, labels, max_length)
    elif task.type == CLASSIFICATION:
        return get_classification_labels(task, labels, max_length, pad_classification)

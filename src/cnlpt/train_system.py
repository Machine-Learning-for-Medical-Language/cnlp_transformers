...
def main(
    ...
    if data_args.weight_classes:
        ...
        class_weights = []
        # class_weights = {task: {label: 1.0 for label in dataset.tasks_to_labels[task]} for task in dataset.tasks_to_labels.keys()}
        for task in dataset.tasks_to_labels.keys():
            labels = dataset.processed_dataset["train"][task]
            weights = []
            if num_labels[task] == 2:
                label_counts = Counter(labels)
                count_0, count_1 = (
                    label_counts[dataset.tasks_to_labels[task][0]],
                    label_counts[dataset.tasks_to_labels[task][1]],
                )
                weights.append(max(1.0, count_1 / count_0))
                weights.append(max(1.0, count_0 / count_1))
                # weight of minority class is determined by severity of class imbalance
            else:
                weights = [None] * num_labels[task]
            if len(dataset.tasks_to_labels.keys()) > 1:
                class_weights.append(weights)
            else:
                class_weights = (
                    weights  # if we just have the one class, simplify it for pytorch
                )
